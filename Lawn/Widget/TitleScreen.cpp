...
{
	g->DrawImageF(IMAGE_PLANTSHADOW, 10000000.0f, 0.0f);
}
...
void TitleScreen::Draw(Graphics* g)
{
	...
	Graphics aClipG(*g);
	if (mMainCounter % 230 < 75)
	{
		aClipG.DrawImage(Sexy::IMAGE_TITLESCREEN_GLOW, 575, 213);
		aClipG.DrawImage(Sexy::IMAGE_TITLESCREEN_BULB, 681, 327);
	}
	...
	{
		g->DrawImage(IMAGE_LOADBAR_GRASS, aGrassX, aGrassY + 8);
	...
	else
	{
		...
		aClipG.DrawImage(IMAGE_LOADBAR_GRASS, aGrassX, aGrassY + 8);
...
void TitleScreen::Update()
{
	...
	for (int i = 0; i < LENGTH(aTriggerPoint); i++)
	{
		if (aPrevWidth < aTriggerPoint[i] && mCurBarWidth >= aTriggerPoint[i])
		{
			...
			float aPosX = aTriggerPoint[i] + 475.0f;
			float aPosY = 511.0f;
			Reanimation* aSproutReanim = mApp->AddReanimation(aPosX, aPosY + 5, 0, aReanimType);
