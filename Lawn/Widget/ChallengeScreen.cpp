...
ChallengeDefinition gChallengeDefs[NUM_CHALLENGE_MODES] = {
	...
	{ GameMode::GAMEMODE_INTRO,                                10,  ChallengePage::CHALLENGE_PAGE_LIMBO,       2,  5,  _S("Intro") }
};
...
ChallengeScreen::ChallengeScreen(LawnApp* theApp, ChallengePage thePage)
{
	...
	mBackButton->Resize(48, 668, 111, 26);
	...
	for (int aPageIdx = CHALLENGE_PAGE_SURVIVAL; aPageIdx < MAX_CHALLANGE_PAGES; aPageIdx++)
	{
		...
		aPageButton->Resize(200 + 280 * aPageIdx, 650, 100, 75);
	...
	for (int aChallengeMode = 0; aChallengeMode < NUM_CHALLENGE_MODES; aChallengeMode++)
	{
		...
		if (aChlDef.mPage == CHALLENGE_PAGE_CHALLENGE || aChlDef.mPage == CHALLENGE_PAGE_LIMBO || aChlDef.mPage == CHALLENGE_PAGE_PUZZLE)
			aChallengeButton->Resize(198 + aChlDef.mCol * 195, 143 + aChlDef.mRow * 129, 104, 115);
		else
			aChallengeButton->Resize(198 + aChlDef.mCol * 195, 145 + aChlDef.mRow * 145, 104, 115);
...
void ChallengeScreen::Draw(Graphics* g)
{
	...
	if (aTrophiesTotal > 0)
	{
		...
		TodDrawString(g, aTrophyString, 939, 73, Sexy::FONT_DWARVENTODCRAFT12, Color(255, 240, 0), DS_ALIGN_CENTER);
	}
	TodDrawImageScaledF(g, Sexy::IMAGE_TROPHY, 918, 74, 0.5f, 0.5f);
