...
void SeedChooserScreen::UpdateViewLawn()
{
	...
	{
		mBoard->Move(-TodAnimateCurve(0, 100, mViewLawnTime, aBoardX, BOARD_OFFSET_Y, CURVE_EASE_IN_OUT), 0);
	...
	else if (mViewLawnTime <= 350)
	{
		...
		mBoard->Move(-TodAnimateCurve(250, 350, mViewLawnTime, BOARD_OFFSET_Y, aBoardX, CURVE_EASE_IN_OUT), 0);
	...
	else if (mSeedsInFlight <= 0)
	{
		...
		else
		{
			...
			else if (aSeedType != mToolTipSeed)
			{
				...
				mToolTip->mX = (SEED_PACKET_WIDTH - mToolTip->mWidth) / 2 + aSeedX;//ClampInt((SEED_PACKET_WIDTH - mToolTip->mWidth) / 2 + aSeedX, 0, BOARD_WIDTH - mToolTip->mWidth);
