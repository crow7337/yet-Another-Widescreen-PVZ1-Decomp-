...
AlmanacDialog::AlmanacDialog(LawnApp* theApp) : LawnDialog(theApp, DIALOG_ALMANAC, true, _S("Almanac"), _S(""), _S(""), BUTTONS_NONE)
{
	...
	mCloseButton->Resize(1176, 687, 89, 26);
	...
	mIndexButton->Resize(10, 687, 164, 26);
	...
	mPlantButton->Resize(370, 417, 156, 42);
	...
	mZombieButton->Resize(724, 415, 210, 48);
	...
	mNextButton->Resize(530, 687, 164, 26);
	...
	mLastButton->Resize(830, 687, 164, 26);
...
void AlmanacDialog::DrawIndex(Graphics* g)
{
	...
	TodDrawString(g, _S("[SUBURBAN_ALMANAC_INDEX]"), BOARD_WIDTH / 2, 105, Sexy::FONT_HOUSEOFTERROR28, Color(220, 220, 220), DrawStringJustification::DS_ALIGN_CENTER);
...
{
	TodDrawString(g, _S("[SUBURBAN_ALMANAC_PLANTS]"), BOARD_WIDTH / 2, 105, Sexy::FONT_HOUSEOFTERROR20, Color(213, 159, 43), DrawStringJustification::DS_ALIGN_CENTER);
		...
		mSelectedSeed == SeedType::SEED_CATTAIL || mSelectedSeed == SeedType::SEED_SEASHROOM)
	{
		...
		g->DrawImage(aNight ? Sexy::IMAGE_ALMANAC_GROUNDNIGHTPOOL : Sexy::IMAGE_ALMANAC_GROUNDPOOL, 910, 137);
	...
	g->DrawImage(Sexy::IMAGE_ALMANAC_PLANTCARD, 859, 136);
	...
	TodDrawString(g, aName, 1010, 335, Sexy::FONT_DWARVENTODCRAFT18YELLOW, Color::White, DS_ALIGN_CENTER);
	TodDrawStringWrapped(g, aDescriptionName, Rect(890, 360, 258, 230), Sexy::FONT_BRIANNETOD12, Color(40, 50, 90), DS_ALIGN_LEFT);
	...
	if (mSelectedSeed != SeedType::SEED_IMITATER)
	{
		...
		TodDrawStringWrapped(g, aCostStr, Rect(890, 576, 134, 50), Sexy::FONT_BRIANNETOD12, Color::White, DS_ALIGN_LEFT);
		...
		TodDrawStringWrapped(g, aRechargeStr, Rect(990, 576, 139, 50), Sexy::FONT_BRIANNETOD12, Color(40, 50, 90), DS_ALIGN_RIGHT);
	}
}
...
void AlmanacDialog::DrawZombies(Graphics* g)
{
	...
	TodDrawString(g, _S("[SUBURBAN_ALMANAC_ZOMBIES]"), BOARD_WIDTH / 2, 105, Sexy::FONT_DWARVENTODCRAFT24, Color(0, 196, 0), DS_ALIGN_CENTER);
	...
	g->DrawImage(mZombie->mZombieType == ZombieType::ZOMBIE_ZAMBONI || mZombie->mZombieType == ZombieType::ZOMBIE_BOBSLED ?
		Sexy::IMAGE_ALMANAC_GROUNDICE : Sexy::IMAGE_ALMANAC_GROUNDDAY, 918, 167);
	if (mZombie && !ZombieHasSilhouette(mZombie->mZombieType))
	{
		...
		case ZombieType::ZOMBIE_BOSS:			aZombieGraphics.TranslateF(-540, -200);	break;
		}
	...
	g->DrawImage(Sexy::IMAGE_ALMANAC_ZOMBIECARD, 859, 136);
	...
	TodDrawString(g, aName, 1015, 420, Sexy::FONT_DWARVENTODCRAFT18GREENINSET, Color(190, 255, 235, 255), DS_ALIGN_CENTER);
	...
	TodDrawStringWrapped(g, aDescription, Rect(890, 445, 278, 170), Sexy::FONT_BRIANNETOD12, Color(40, 50, 90), aAlign);
}
...
void AlmanacDialog::GetSeedPosition(SeedType theSeedType, int& x, int& y)
{
	...
	{
		int aFinalSeedType = mIndexedPage == 0 ? theSeedType : theSeedType - 48;
		x = aFinalSeedType % 8 * 52 + 56;
		y = aFinalSeedType / 8 * 78 + 160;
	}
}
...
void AlmanacDialog::GetZombiePosition(ZombieType theZombieType, int& x, int& y)
{
	...
	else
	{
		...
		x = aFinalZombeType % 5 * 85 + 56;
		y = aFinalZombeType / 5 * 80 + 162;
	}
}
