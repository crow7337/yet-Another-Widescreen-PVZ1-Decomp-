...
void CutScene::AnimateBoard()
{
	...
	{
		//int aSeedBankX = CalcPosition(aTimeSeedBankRightStart, aTimeSeedBankRightEnd, BOARD_ADDITIONAL_WIDTH, SEED_BANK_OFFSET_X_END + BOARD_ADDITIONAL_WIDTH);
...
void CutScene::Update()
{
	...
	if (mCutsceneTime >= aTimeStart)
	{
		...
		ShowShovel();
		mApp->StartPlaying();
		// The game is haunted and moves the seed bank to a random position after starting, so this is done to prevent that
		// Fun fact: this also recreates the extra padding between the shovel button and seed bank from console versions
