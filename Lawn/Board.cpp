...
void Board::PickZombieWaves()
{
	...
	if (mApp->IsAdventureMode())
	{
		...
		{
			mNumWaves = gZombieWaves[ClampInt(mLevel - 1, 0, 59)];
...
Reanimation* Board::CreateBushReanim(float theX, float theY, int theRenderOrder, ReanimationType theReanimationType)
{
	Reanimation* aReanim = mApp->mEffectSystem->mReanimationHolder->AllocReanimation(theX, theY, theRenderOrder, theReanimationType);
...
void Board::RustleBush(int theRow)
{
	...
	int aRow = ClampInt(theRow, 0, 4);
	TOD_ASSERT(mApp->ReanimationGet(mBushReanimIDs[aRow]));
